...
public class MDocumentList extends MAbstractView implements MOutputView {
	...
	private void onPaste() {
		...
		for (int i = 0; i < copiedDocumentList.size(); ++i) {
			...
			/*
			 * String insertedObjectStr = MMongoUtil.toJson(
			 * dataManager.getDB(), data, true ); debug( insertedObjectStr );
	...
	public void init2() {
		...
		// fd_naviLabel.left = new FormAttachment(prevItemsButton, -66,
		// SWT.LEFT);
		...
		if (items.length == 1) {
			try {
				...
				if (documentEditor == null) {
					documentEditor = (MDocumentEditor) PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage()
							.showView(MDocumentEditor.class.getName());
				}
	...
	private void updateDocument(TableItem item, int columnIndex, Class clazz, String value) {
		 ...
		 * objectIdStr ) ); BasicDBObject update = new BasicDBObject( "$set",
		 * new BasicDBObject( fieldName, newValue ) );
		 * 
		 * String updateStr = null; if( newValue.getClass() == Integer.class ) {
		 * updateStr = "{ $set : { \"" + fieldName + "\" : Number} }"; } else {
		 * updateStr = MMongoUtil.toJson( dataManager.getDB(), update ); }
		 * 
		 * actionManager.executeAction( "db." + collName + ".update(" +
		 * MMongoUtil.toJson( dataManager.getDB(), query ) + "," + updateStr +
		 * ",false, false )" );
		 */
	...
	private void onTableColumnSelect(TableColumn column) {
		...
		/*
		 * //sort order if( sortOrder == 1 ) { sortOrder = -1; } else {
		 * sortOrder = 1; }
		 * 
		 * final int _sortOrder = sortOrder;
		 * 
		 * drawTable( dataManager.getDocumentDataList(),
		 * dataManager.getColumnNameList() );
		 */
	}
		...
		shell.getDisplay().asyncExec(new Runnable() {
			public void run() {// *****
				...
				try {
					...
					// MSwtUtil.getNameListFromDataList( _dataList );
					// debug( columnNameList );

					{
						Collections.sort(_columnNameList, new Comparator() {
						...
						// MSwtUtil.getTableColumnWidthFromProperties(
						// getTablePrefix() , table, prop, new int[]{ 40, 40,
						...
						for (int i = 0; i < _dataList.size(); ++i) {
							...
							for (int k = 0; k < _columnNameList.size(); ++k) {
								...
								if (value == null) {
									 ...
									 * "yyyy/MM/dd HH:mm:ss" ); item.setText( k,
									 * df.format( ( Date )value ) ); } else
									 */
									{
